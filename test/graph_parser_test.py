...
def test_shortest_distance_is_correct(initialise_graph):
    assert 5 == initialise_graph.get_distance(TEST_END_POINT)


# TODO: find an example where the result is always the same
def test_get_right_path(initialise_graph):
    assert initialise_graph.get_path(TEST_END_POINT) == TEST_PATH_POSSIBLE_RESULT_1 \
           or initialise_graph.get_path(TEST_END_POINT) == TEST_PATH_POSSIBLE_RESULT_2
